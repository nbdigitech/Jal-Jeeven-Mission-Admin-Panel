...
export default function EmailsDrawer({
  open,
  onOpenChange,
}: EmailsDrawerProps) {
    ...
    <Drawer open={open} onOpenChange={onOpenChange}>
      <DrawerContent className="h-[90vh]">
        <DrawerHeader className="flex justify-between items-center border-b px-6 py-4">
          ...
          <Link href="/emails">
            <Button className="bg-[#136FB6] text-white hover:bg-[#e06a0a]">
              View All
            </Button>
          </Link>
        </DrawerHeader>
        ...
        <ScrollArea className="flex-1 px-6">
          <div className="space-y-2 py-4">
            {emails.slice(0, 5).map((email) => (
              <Card
                key={email.id}
                className={`rounded-lg cursor-pointer transition hover:border-[#136FB6] ${
                  !email.read ? "bg-[#136FB6]/5" : ""
                }`}
              >
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="flex gap-1 flex-shrink-0">
                      ...
                      >
                        <Star
                          className={`w-4 h-4 ${
                            email.starred ? "fill-[#136FB6] text-[#136FB6]" : ""
                          }`}
                        />
                      </button>
                      <button
